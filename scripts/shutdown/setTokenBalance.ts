...
import { getImpersonatedSignerForge } from '../../test/helpers';
...
async function main(cToken: string, giveTo: string, amount: string) {
  ...
  const topHolder = cTokenHolders[cToken as keyof typeof cTokenHolders][0];
  ...
  const cTokenContract = new ethers.Contract(cToken, IERC20__factory.createInterface(), provider) as IERC20;
  const topHolderActualBalance = await cTokenContract.balanceOf(topHolder);

  if (topHolderActualBalance.lt(amount))
    throw new Error(
      `Requested ${amount.toString()} of cToken ${cToken}, but top holder only has ${topHolderActualBalance.toString()}`
    );

  const unsignedTx = await cTokenContract.populateTransaction.transfer(giveTo, amount);
